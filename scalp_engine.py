...
VERSION: 4.0 (PRODUCTION QUANT - LSE MACRO HOOK + FAIL-SAFE AUDIT & CONCURRENCY)
===============================================================================
"""
...
# ================= 3. LSE MACRO TELEMETRY HOOK =================
def get_lse_yield_impact():
    """
    Reads the decoupled lse_macro.csv generated by lse_sync.py
    Provides US10Y bond yield and real-time gold macro bias.
    """
    if os.path.exists(LSE_MACRO_FILE):
        try:
            ...
            if not df.empty and "gold_macro_impact" in df.columns:
                impact = str(df["gold_macro_impact"].iloc[-1]).strip()
                val = float(df["us10y_yield"].iloc[-1])
                return impact, val
        except Exception as e:
            print(f"[LSE_HOOK_ERROR] Failed reading macro telemetry: {e}")
    return "NEUTRAL", 0.0

# ================= 4. 30M REGIME & INSTITUTIONAL MONEY FLOW =================
...
# ================= 5. MATHEMATICAL KERNELS & DERIVATIVES =================
...
# ================= 6. QUANT SCALP & TAIL-RISK DISCRIMINATION =================
...
# ================= 7. AUTOMATED TRADE MONITOR & AUDIT =================
...
def run():
    ...
    if decision["signal"]:
        ...
        # Read isolated LSE Macro Telemetry
        lse_impact, us10y_val = get_lse_yield_impact()
        ...
        macro_line = f"`{us10y_val:.2f}% ({lse_impact})`" if us10y_val > 0 else f"`{lse_impact}`"

        card = (
            f"{icon}\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"🌐 *30M Regime Vector*: `{regime_30m}`\n"
            f"🏛 *Macro (US10Y Yield)*: {macro_line}\n"
